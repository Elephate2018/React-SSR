...
class Detail extends Component {
  ...
  setStateFun() {
    ...
    let title = this.props.location.pathname.split('/')[2]
    console.log(page, 'and ', title)
    var datas;
    ...
    return datas.filter(item => item.title.replace(/ /g, "_") == title)
  ...
  render() {
    let page = this.props.location.pathname.split('/')[1]
    let title = this.props.location.pathname.split('/')[2]
    console.log(page, 'and from asdsadsda ', title)
    return (
      <div>
        <h1>Details</h1>
        {
          this.setStateFun().map((item, index) => {
            return (
              <section key={index}>
